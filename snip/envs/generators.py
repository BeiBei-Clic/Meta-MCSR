...
class RandomFunctions(Generator):
    ...
    def _generate_datapoints(
        ...
        ### modified (normalized y)
        if self.params.normalize_y:
            eps = 1e-10
            outputs = np.divide( np.subtract(outputs, min(outputs)) , max(outputs) - min(outputs) + eps )
            
        return inputs, outputs

    def _generate_datapoints_LSO(
        self,
        tree,
        n_points,
        rng,
        ...
        while trials < max_trials:
            ...
            # input = rng.uniform(input_range[0][0], input_range[0][1], size=(sample, input_dimension))
            min_val = -10
            max_val = 10
            delta_x = np.random.randint(1,10)                 
            x_min = np.random.randint(min_val , max_val-delta_x)
            x_max = x_min + delta_x
            # x_max = np.random.randint(x_min+1, min(x_min+1, max_val+1))
            input = np.linspace(x_min, x_max , n_points).reshape(-1,1)
        
            # breakpoint()
            # print("input LSO: ", input)
            output = tree.val(input)
            # print("output: ", output)
            ## get rid of nans
            is_nan_idx = np.any(np.isnan(output), -1)
    ...
    def generate_datapoints(
        ...
        **kwargs,
        ...
        # inputs, outputs = self._generate_datapoints_LSO(
        #     tree=tree,
        #     n_points=n_input_points,
        #     **kwargs,
        # )
...
if __name__ == "__main__":
    ...
    from snip.envs.environment import SPECIAL_WORDS
