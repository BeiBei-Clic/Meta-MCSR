...
- **灵活的MCTS模式**: 支持奖励网络增强的MCTS和传统MCTS，可通过参数灵活切换
- **智能模式回退**: 当奖励网络模型不可用时自动切换到传统MCTS，确保系统稳定性
- **明确的模式标识**: 运行时会明确显示当前使用的MCTS模式，便于用户理解和调试
...
3. **运行增强MCTS**
   ```bash
   # 复合函数测试（默认使用奖励网络增强的MCTS）
   python mcts_with_reward_network.py composite
   
   # 指定数据集文件测试（默认使用奖励网络增强的MCTS）
   ...
   python mcts_with_reward_network.py dataset-file --dataset-path your_data.npz --use-reward-network false
   ```

**重要改进**: 
- 两种测试模式现在都自动进行训练/测试集分割，提供更可靠的泛化性能评估！
- **默认使用奖励网络增强的MCTS**，可通过参数控制切换到传统MCTS
- 添加了`--use-reward-network`和`--no-use-reward-network`参数来明确控制MCTS模式
...
- 特征数: 2个输入特征 + 1个输出标签
- 算法性能: 默认使用奖励网络增强的MCTS，可获得R2 > 0.8的优秀拟合效果
- 训练/测试分割: 自动进行80%训练，20%测试分割，评估泛化性能

**默认行为说明**：
- **默认模式**: 使用奖励网络增强的MCTS，利用深度学习模型预测表达式潜力
- **如需切换**: 使用`--no-use-reward-network`参数切换到传统MCTS模式
- **模型要求**: 奖励网络模式需要预训练的模型文件，否则会自动回退到传统MCTS

**输出说明**：
- `训练集 R2/RMSE`: 在训练集上的性能指标
- `测试集 R2/RMSE`: 在测试集上的性能指标（更重要的泛化指标）
- 过拟合检测: 自动检测训练-测试性能差值，警告潜在的过拟合问题
- **模式标识**: 运行时会明确显示当前使用的MCTS模式（增强或传统）
...
- `--no-use-reward-network`: 直接使用传统MCTS（不使用奖励网络）
...
python mcts_with_reward_network.py composite --no-use-reward-network
# 或者
python mcts_with_reward_network.py composite --use-reward-network false
```
...
```bash
python mcts_with_reward_network.py dataset-file \
    --dataset-path your_complex_data.npz \
    --max-iterations 2000 \
    --max-depth 10 \
    --train-test-split 0.8 \
    --use-reward-network true \
...
**传统MCTS模式（适用于简单问题或快速原型）：**
```bash
python mcts_with_reward_network.py dataset-file \
    --dataset-path your_simple_data.npz \
    --max-iterations 500 \
    --max-depth 6 \
    --train-test-split 0.8 \
    --no-use-reward-network
...
# 传统MCTS
python mcts_with_reward_network.py composite --no-use-reward-network
python mcts_with_reward_network.py dataset-file --dataset-path data.npz --use-reward-network false
...
- **数据集**: `--dataset-path 文件路径`
- **搜索参数**: `--max-iterations`, `--max-depth`
- **数据分割**: `--train-test-split 比例`
- **模型路径**: `--expr-encoder`, `--reward-network`

### 默认行为
- **MCTS模式**: 默认使用奖励网络增强MCTS
- **训练/测试分割**: 默认80%训练，20%测试
- **模型回退**: 奖励网络模型不可用时自动切换到传统MCTS
- **模式显示**: 运行时会明确显示当前使用的MCTS模式
...
### 5. MCTS模式选择
- **默认使用奖励网络**: 系统默认使用奖励网络增强的MCTS，通常效果更好
- **切换到传统模式**: 使用`--no-use-reward-network`或`--use-reward-network false`
- **模型文件缺失**: 如果奖励网络模型文件不存在，系统会自动切换到传统MCTS
- **性能对比**: 奖励网络模式通常在复杂函数上表现更优，传统模式在简单任务上速度更快

### 6. 奖励网络相关问题
- **模型文件要求**: 奖励网络模式需要`weights/expression_encoder`和`weights/reward_network_final`文件
- **自动回退**: 当模型文件缺失或加载失败时，系统会自动切换到传统MCTS
- **模式识别**: 运行时会明确显示当前使用的MCTS模式，便于调试和验证